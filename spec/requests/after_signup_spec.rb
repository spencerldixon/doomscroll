...
RSpec.describe "AfterSignup", type: :request do
  let(:user) do
    User.create!(
  ...
  describe "PUT /after_signup/name" do
    it "saves the zine name and moves to the feeds step" do
      put after_signup_path(:name), params: { zine_name: "My Zine" }

      expect(response).to redirect_to(after_signup_path(:feeds))
      expect(user.reload.zine_preference.zine_name).to eq("My Zine")
    end

    it "re-renders the step when zine name is blank" do
      put after_signup_path(:name), params: { zine_name: "" }
  ...
  describe "PUT /after_signup/feeds" do
    it "saves the feeds and moves to the delivery step when at least 3 feeds selected" do
      put after_signup_path(:feeds), params: { feed_ids: feeds.first(3).map(&:id) }

      expect(response).to redirect_to(after_signup_path(:delivery))
    ...
    it "re-renders the step when fewer than 3 feeds selected" do
      put after_signup_path(:feeds), params: { feed_ids: feeds.first(2).map(&:id) }
  ...
  describe "PUT /after_signup/delivery" do
    ...
    it "rejects an invalid delivery day" do
      put after_signup_path(:delivery), params: { delivery_day: 9 }
